...
private struct MainTabsView: View {
    ...
    var body: some View {
        TabView(selection: $selectedTab) {
            ChatView().tabItem { Label("聊天", systemImage: "message") }.tag(0)
            AgendaView().tabItem { Label("日程", systemImage: "calendar") }.tag(1)
            ScheduleView(routedIntent: $scheduleRoute).tabItem { Label("课表", systemImage: "calendar.badge.clock") }.tag(2)
...
private struct ChatView: View {
    ...
    private func sendText() { let value = text.trimmingCharacters(in: .whitespacesAndNewlines); guard !value.isEmpty, let memberID = env.session.currentMemberID else { return }; env.send(ChatMessageModel(senderID: memberID, body: value, kind: .text, status: .sending, replyToID: replyTo?.id)); text = ""; replyTo = nil }
    private func importSelectedImage(_ item: PhotosPickerItem) async {
        do {
            ...
            env.send(ChatMessageModel(senderID: memberID, body: "图片", kind: .image, status: .sending, mediaPath: path, replyToID: replyTo?.id))
            replyTo = nil
        } catch {
...
private struct MessageRow: View {
    ...
    @ViewBuilder private var messageContent: some View { switch message.kind { case .image: if let image = image { Button(action: previewImage) { Image(uiImage: image).resizable().scaledToFit().frame(maxWidth: 230, maxHeight: 190).clipShape(RoundedRectangle(cornerRadius: 16, style: .continuous)) }.buttonStyle(.plain).accessibilityLabel("全屏查看图片消息") } else { Button(action: loadRemoteMedia) { Label(isLoadingRemoteMedia ? "正在加载图片" : "图片不可用", systemImage: "photo") }.disabled(isLoadingRemoteMedia || env.remoteChatMediaResolver == nil) }; case .audio: Button { if let path = message.mediaPath { do { try playback.toggle(messageID: message.id, url: env.mediaStore.url(for: path)) } catch { env.lastError = "语音不可播放：\(error.localizedDescription)" } } else { loadRemoteMedia() } } label: { VStack(alignment: .leading, spacing: 4) { Label(isLoadingRemoteMedia ? "正在加载语音" : audioLabel, systemImage: playback.isPlaying(message.id) ? "pause.circle.fill" : "play.circle.fill"); ProgressView(value: playback.progress(for: message.id)).frame(width: 150) } }.disabled(isLoadingRemoteMedia || (message.mediaPath == nil && env.remoteChatMediaResolver == nil)); default: Text(message.body).textSelection(.enabled) } }
...
struct AgendaView: View {
    ...
    var body: some View { NavigationStack { List {
        if items.isEmpty {
            ContentUnavailableView("没有日程", systemImage: "calendar", description: Text("点击右上角加号添加日程、考试、作业或点菜。"))
        } else {
