...
enum RepositoryError: LocalizedError { case versionConflict, forbidden, invalidData, recallExpired, importUndoConflict
    var errorDescription: String? { switch self { case .versionConflict: return "这条备忘录已被其他成员修改，请查看最新内容后再保存。"; case .forbidden: return "当前成员没有执行此操作的权限。"; case .invalidData: return "数据无效，请检查日期、周次和时间。"; case .recallExpired: return "消息发送超过 5 分钟，不能撤回。"; case .importUndoConflict: return "导入后的课程已被修改或删除，不能安全撤销。请手动核对后处理。" } }
}
...
@MainActor final class AppleCalendarExporter {
    ...
    func add(_ item: AgendaItemModel) async throws {
        guard let timing = Self.timing(for: item) else { throw AppleCalendarExportError.notTimed }
...
@MainActor final class LocalChatRepository: ChatRepository {
    ...
    func create(_ message: ChatMessageModel) throws {
        ...
        if (message.kind == .image || message.kind == .audio) && writes.isRemoteEnabled {
    ...
    func retry(_ message: ChatMessageModel, by memberID: String) throws { guard message.senderID == memberID, message.status == .failed, try MemberDirectory.containsActive(memberID, in: context) else { throw RepositoryError.forbidden }; message.statusRaw = ReceiptStatus.sending.rawValue; try context.save() }
...
@MainActor final class LocalAgendaRepository: AgendaRepository {
    ...
    func delete(exception: AgendaExceptionModel, for item: AgendaItemModel, by memberID: String) throws { guard item.creatorID == memberID, exception.agendaID == item.id, try MemberDirectory.containsActive(memberID, in: context) else { throw RepositoryError.forbidden }; try writes.commit(changing: { context.delete(exception) }, intents: { [RemoteMutationIntent(entityType: .agendaException, entityID: exception.id, operation: .delete)] }) }
...
@MainActor final class LocalMemoRepository: MemoRepository {
    ...
    func delete(_ memo: MemoModel, by memberID: String) throws { guard memo.creatorID == memberID, try MemberDirectory.containsActive(memberID, in: context) else { throw RepositoryError.forbidden }; try writes.commit(changing: { context.delete(memo) }, intents: { [RemoteMutationIntent(entityType: .memo, entityID: memo.id, operation: .delete)] }) }
}
...
/// Shared initial map coordinates. They keep the first map view, seeded
/// member locations and the new-place fallback in the same Xi'an area.
...
struct DemoLocationProvider: LocationProvider {
    func locations(now: Date) -> [LocationSnapshotModel] {
        [
            LocationSnapshotModel(memberID: MemberID.sendai.rawValue, latitude: FamilyMapDefaults.shaanxiNormalUniversityChangAn.latitude, longitude: FamilyMapDefaults.shaanxiNormalUniversityChangAn.longitude, timestamp: now.addingTimeInterval(-12 * 60), event: "到达学校", horizontalAccuracy: 35, source: .automatic),
            LocationSnapshotModel(memberID: MemberID.osaka.rawValue, latitude: FamilyMapDefaults.xianJiaotongUniversityXingQing.latitude, longitude: FamilyMapDefaults.xianJiaotongUniversityXingQing.longitude, timestamp: now.addingTimeInterval(-28 * 60), event: "到达学校", horizontalAccuracy: 45, source: .automatic),
            LocationSnapshotModel(memberID: MemberID.kyoto.rawValue, latitude: FamilyMapDefaults.changAnVankePlaza.latitude, longitude: FamilyMapDefaults.changAnVankePlaza.longitude, timestamp: now.addingTimeInterval(-45 * 60), event: "到达长安万科广场", horizontalAccuracy: 40, source: .automatic)
        ]
    }
}
...
@MainActor @Observable final class SessionStore {
    ...
    init() { currentMemberID = UserDefaults.standard.string(forKey: key).flatMap { $0.isEmpty ? nil : $0 } }
...
@MainActor @Observable final class AppEnvironment {
    ...
    let runtimeMode: AppRuntimeMode = .localOnly
    ...
    let remoteChatMediaResolver: (any RemoteChatMediaResolving)?
    ...
    let chatRepository: ChatRepository; let chatTransport: DemoChatTransport; let semesterRepository: SemesterRepository; let agendaRepository: AgendaRepository; let scheduleRepository: ScheduleRepository; let memoRepository: MemoRepository; let noticeRepository: NoticeRepository; let locationRepository: LocationRepository
    ...
    init(modelContext: ModelContext, remoteChatMediaResolver: (any RemoteChatMediaResolving)? = nil, recoveryFlow: RecoveryFlow? = nil, registrationFlow: RegistrationFlow? = nil) {
        context = modelContext
        mediaStore = LocalMediaStore()
        self.remoteChatMediaResolver = remoteChatMediaResolver
        ...
        chatTransport = DemoChatTransport(context: modelContext)
        semesterRepository = LocalSemesterRepository(context: modelContext)
        agendaRepository = LocalAgendaRepository(context: modelContext)
        scheduleRepository = LocalScheduleRepository(context: modelContext)
        memoRepository = LocalMemoRepository(context: modelContext)
        noticeRepository = LocalNoticeRepository(context: modelContext)
        let locations = LocalLocationRepository(context: modelContext)
    ...
    func locationScenePhaseChanged(_ phase: ScenePhase) { automaticLocationService.scenePhaseChanged(phase) }
