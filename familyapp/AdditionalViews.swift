...
private struct WeeklyScheduleGrid: View {
        ...
        GeometryReader { proxy in
            ...
            ZStack(alignment: .topLeading) {
                ...
                ForEach(placements) { placement in
                    placementView(placement, labelWidth: labelWidth, headerHeight: headerHeight, column: column, row: row)
                }
            }
        }
    ...
    private func placementView(_ placement: Placement, labelWidth: CGFloat, headerHeight: CGFloat, column: CGFloat, row: CGFloat) -> some View {
        let blockWidth: CGFloat = max(0, (column - 4) / CGFloat(placement.laneCount))
        let periodCount = placement.periods.upperBound - placement.periods.lowerBound + 1
        let blockHeight: CGFloat = max(0, CGFloat(periodCount) * row - 4)
        let x: CGFloat = labelWidth + CGFloat(placement.day) * column + 2 + CGFloat(placement.lane) * blockWidth
        let y: CGFloat = headerHeight + CGFloat(placement.periods.lowerBound) * row + 2
        return Button {
            onSelect(placement.occurrence)
        } label: {
            GridScheduleBlock(item: placement.occurrence, showDetails: showDetails, isInGap: placement.isInGap)
        }
        .buttonStyle(.plain)
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(availableMemberIDs, id: \.self) { memberID in
                    ...
                    .accessibilityValue(selectedMemberID == memberID ? "已选中" : "未选中")
                }
            }
            .padding(6)
        }
...
struct MoreView: View {
    ...
    var body: some View { NavigationStack { List {
        Section("账户与安全") {
            if env.runtimeMode == .remoteSync, let flow = env.recoveryFlow {
                NavigationLink { RecoverySetupView(flow: flow) } label: { Label("恢复助记词", systemImage: "key.horizontal") }
            }
            if env.runtimeMode == .remoteSync, let flow = env.registrationFlow, flow.canManageDevices {
                NavigationLink { LoggedInDevicesView(flow: flow) } label: { Label("已登录设备", systemImage: "desktopcomputer.and.iphone") }
            }
            NavigationLink { SettingsView() } label: { Label("个人资料与账户设置", systemImage: "person.crop.circle") }
        }
        Section("家庭管理") {
            if env.runtimeMode == .remoteSync, let registrationFlow = env.registrationFlow {
                ...
                if registrationFlow.canReviewApplications {
                    NavigationLink { JoinRequestListView(flow: registrationFlow) } label: { Label("加入申请", systemImage: "person.badge.clock") }
                }
            } else {
        ...
        Section("设置") {
            NavigationLink { SettingsView(focus: .appearance) } label: { Label("外观", systemImage: "circle.lefthalf.filled") }
            NavigationLink { SettingsView(focus: .privacy) } label: { Label("隐私与定位", systemImage: "hand.raised") }
        }
    }.navigationTitle("更多") } }
}
...
struct LocalFamilyDirectoryView: View {
...
private struct GlobalSearchView: View {
    ...
    private var matchingAgendas: [AgendaItemModel] { agendas.filter { $0.deletedAt == nil && $0.purgedAt == nil && matches($0.title, $0.location, $0.note, $0.dishes) } }
    private var matchingMemos: [MemoModel] { memos.filter { $0.deletedAt == nil && $0.purgedAt == nil && matches($0.title, $0.content) } }
    private var matchingNotices: [NoticeModel] { notices.filter { $0.deletedAt == nil && $0.purgedAt == nil && matches($0.title, $0.content) } }
...
struct NoticeListView: View {
    ...
    var sorted: [NoticeModel] { notices.filter { $0.deletedAt == nil && $0.purgedAt == nil }.sorted { lhs, rhs in lhs.pinned == rhs.pinned ? (lhs.pinned ? lhs.updatedAt > rhs.updatedAt : lhs.createdAt > rhs.createdAt) : lhs.pinned && !rhs.pinned } }
...
private enum SettingsFocus { case all, appearance, privacy }
...
private struct SettingsView: View {
    ...
    var body: some View { Form {
        ...
        if focus == .privacy {
            Section("业务内容与 AI") {
                Toggle("允许业务内容发送 AI", isOn: $allowContent).disabled(!enabled)
                Text("AI 默认不参与业务；聊天和地图不会发送给 AI。")
                    .font(.caption).foregroundStyle(.secondary)
            }
        }
        if focus == .all {
        ...
        Section { Button("退出登录", role: .destructive) {
            Task { @MainActor in
                do { try await env.logoutCurrentSession() }
                catch { env.lastError = error.localizedDescription }
            }
        } }
        Section("数据管理") {
    ...
    private var settingsTitle: String {
        switch focus { case .all: "设置"; case .appearance: "外观"; case .privacy: "隐私与定位" }
    }
...
private struct ProfileEditor: View {
    ...
    var body: some View { Form { TextField("昵称", text: $nickname); Picker("头像", selection: $avatar) { ForEach(choices, id: \.self) { Image(systemName: $0).tag($0) } } }.navigationTitle("个人资料").toolbar { ToolbarItem(placement: .confirmationAction) { Button("保存") { saveProfile() } }; ToolbarItem(placement: .cancellationAction) { Button("取消") { dismiss() } } }.onAppear { nickname = profile.nickname; avatar = profile.avatarSymbol ?? choices[0] } }

    private func saveProfile() {
        let trimmed = nickname.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !trimmed.isEmpty else { env.lastError = "昵称不能为空。"; return }

        let previousNickname = profile.nickname
        let previousAvatar = profile.avatarSymbol
        profile.nickname = trimmed
        profile.avatarSymbol = avatar
        do {
            try env.context.save()
            dismiss()
        } catch {
            profile.nickname = previousNickname
            profile.avatarSymbol = previousAvatar
            env.lastError = error.localizedDescription
        }
    }
}
