...
protocol RemoteMediaAPI: Sendable {
    func requestMediaUpload(mediaID: UUID, mimeType: String, sizeBytes: Int, checksum: String) async throws -> RemoteMediaUploadGrant
...
actor RemoteAPIClient: RemoteSyncAPI, RemoteMediaAPI, RemoteImportBatchRollbackAPI, RemoteRecoveryAuthenticatedAPI {
    ...
    func requestMediaUpload(mediaID: UUID, mimeType: String, sizeBytes: Int, checksum: String) async throws -> RemoteMediaUploadGrant {
        struct Body: Encodable {
            let mediaID: UUID; let mimeType: String; let sizeBytes: Int; let checksum: String
            enum CodingKeys: String, CodingKey { case mediaID = "media_id", mimeType = "mime_type", sizeBytes = "size_bytes", checksum }
        }
        return try await request(path: ["v1", "media"], method: "POST", body: Body(mediaID: mediaID, mimeType: mimeType, sizeBytes: sizeBytes, checksum: checksum))
    }
...
private struct RemoteMediaUploadMaterial: Sendable {
    let data: Data
    let checksum: String
}

private enum RemoteMediaFilePreparation {
    ...
    static func load(from url: URL) throws -> RemoteMediaUploadMaterial {
        ...
        guard !data.isEmpty else {
            throw RemoteSyncError.unsupportedLocalData("聊天媒体文件为空")
        }
...
final class RemoteChatMediaTransferCoordinator {
    ...
    func transferAndQueue(_ message: ChatMessageModel) async throws {
        guard message.kind == .image || message.kind == .audio,
        ...
        let mimeType = message.kind == .image ? "image/jpeg" : "audio/m4a"
        ...
        do {
            ...
            try context.save()
            let grant = try await api.requestMediaUpload(mediaID: mediaID, mimeType: mimeType, sizeBytes: material.data.count, checksum: material.checksum)
            guard grant.mediaID == mediaID, grant.status == "pending" || grant.status == "ready" else {
                throw RemoteSyncError.invalidResponse
            }
            if grant.status == "pending" {
                guard let uploadURL = grant.uploadURL else { throw RemoteSyncError.invalidResponse }
            ...
            guard finalized.mediaID == mediaID, finalized.status == "ready", message.recalledAt == nil else {
                throw RemoteSyncError.invalidResponse
            }
    ...
    func resumeRecoverableTransfers() async {
        ...
        for message in messages where message.mediaTransferState == .pending || message.mediaTransferState == .uploading {
...
/// authorized short-lived URL only when a picture is viewed or audio is played
...
final class RemoteChatMediaResolver: RemoteChatMediaResolving {
    ...
    func ensureCachedMedia(messageID: UUID) async throws -> String {
        guard let message = try context.fetch(FetchDescriptor<ChatMessageModel>()).first(where: { $0.id == messageID }),
              ...
              message.kind == .image || message.kind == .audio else {
            throw RemoteSyncError.unsupportedLocalData("聊天媒体不可用")
        }
        if let localPath = message.mediaPath { return localPath }
        let data = try await api.downloadMedia(mediaID: remoteMediaID)
        guard !data.isEmpty else { throw RemoteSyncError.invalidResponse }
        let localPath = message.kind == .image ? try mediaStore.storeImage(data) : try mediaStore.storeAudio(data)
...
enum RemoteBusinessPayload {
    ...
    static func message(_ value: ChatMessageModel) throws -> [String: RemoteJSONValue] {
        ...
        if value.kind == .image || value.kind == .audio {
...
private struct RemoteRecordState: Sendable, Equatable {
    ...
    static func from(change: RemoteChange, entityType: RemoteEntityType) throws -> RemoteRecordState {
        ...
        guard change.version == 1 else { throw RemoteSyncError.malformedChange("缺少实体的历史版本") }
    ...
    func applying(_ change: RemoteChange) throws -> RemoteRecordState {
        ...
        case .create, .update, .upsert:
            ...
            guard !isTombstone || (change.operation == .upsert && entityType == .agendaParticipant) else {
...
private final class RemoteBusinessChangeApplier {
    ...
    private func apply(_ state: RemoteRecordState) throws {
        ...
        // No matching local business model exists yet for backend-only media
        // metadata or geofence audit events. They stay diagnostic mirror rows,
        // rather than inventing a lossy local representation.
    ...
    private func applyMessage(_ state: RemoteRecordState, _ fields: RemoteFields) throws {
        ...
        if remoteMediaID == nil { item.mediaPath = nil }
        insertIfNeeded(item)
    }
...
final class RemoteSyncCoordinator {
    ...
    func synchronize() async throws -> RemoteSyncRunSummary {
        ...
        let pushSummary = try await pushAllPending()
        try await submitPendingImportRollbacks()
        let pullSummary = try await pullAll()
        return RemoteSyncRunSummary(bootstrapped: bootstrapped, pushed: pushSummary.acknowledged, conflicts: pushSummary.conflicts, pulledChanges: pullSummary)
    }
