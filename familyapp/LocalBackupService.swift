...
enum LocalBackupError: LocalizedError {
    case unreadableFile, unsupportedVersion, invalidArchive(String), restoreFailed
    ...
    private func validate(_ archive: FamilyBackupArchive) throws {
        ...
        guard unique(noticeReadPairs),
              value.agendas.allSatisfy({
                  ...
                  Set($0.participantIDs).isSubset(of: knownMembers) && !$0.participantIDs.isEmpty
              }),
...
nonisolated struct MemoBackup: Codable, Sendable { let id: UUID; let title: String?; let content, creatorID: String; let pinned: Bool; let version: Int; let createdAt, updatedAt: Date; let updatedBy: String; @MainActor init(_ value: MemoModel) { id = value.id; title = value.title; content = value.content; creatorID = value.creatorID; pinned = value.pinned; version = value.version; createdAt = value.createdAt; updatedAt = value.updatedAt; updatedBy = value.updatedBy }; @MainActor var model: MemoModel { MemoModel(id: id, title: title, content: content, creatorID: creatorID, pinned: pinned, version: version, createdAt: createdAt, updatedAt: updatedAt, updatedBy: updatedBy) } }
nonisolated struct NoticeBackup: Codable, Sendable { let id: UUID; let title, content, publisherID: String; let pinned: Bool; let createdAt, updatedAt: Date; let isEdited: Bool; @MainActor init(_ value: NoticeModel) { id = value.id; title = value.title; content = value.content; publisherID = value.publisherID; pinned = value.pinned; createdAt = value.createdAt; updatedAt = value.updatedAt; isEdited = value.isEdited }; @MainActor var model: NoticeModel { NoticeModel(id: id, title: title, content: content, publisherID: publisherID, pinned: pinned, createdAt: createdAt, updatedAt: updatedAt, isEdited: isEdited) } }
