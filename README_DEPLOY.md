...
and enable the chosen timers. Cleanup calls existing MediaStorage cleanup only;
it does not use public API requests and exits cleanly when media storage is
unconfigured. Docker restart policies recover Caddy/API/local PostgreSQL after
