...
class SyncService:
    ...
    def _active_snapshot_statement(entity_type: str, model: type[models.Base]):
        ...
        if entity_type in VERSIONED_TYPES:
    ...
    async def _apply(self, actor_id: UUID, device_id: UUID, mutation: MutationIn) -> MutationAck:
        ...
        existing = await self.session.get(model, mutation.entity_id)
    ...
    async def _apply_versioned(self, actor_id: UUID, device_id: UUID, model: type[models.Base], existing: models.Base | None, mutation: MutationIn) -> MutationAck:
        ...
        if mutation.operation == "delete":
            ...
            record.deleted_at = datetime.now(UTC)  # type: ignore[attr-defined]
            record.version = current_version + 1  # type: ignore[attr-defined]
            await self.session.flush()
            await self._soft_delete_dependents(record, mutation.entity_type)
    ...
    def _payload(self, model: type[models.Base], source: dict[str, Any]) -> dict[str, Any]:
        allowed = {column.key for column in model.__table__.columns} - {"id", "version", "created_at", "updated_at", "deleted_at", "password_hash", "token_hash"}
    ...
    async def _soft_delete_dependents(self, record: models.Base, entity_type: str) -> None:
        ...
        if entity_type == "semester":
            ...
            await collect("foodRead", models.FoodRead, models.FoodRead.agenda_id, record.id)  # type: ignore[attr-defined]
        ...
        for child_type, child in children:
            await self._soft_delete_dependents(child, child_type)
            child.deleted_at = datetime.now(UTC)  # type: ignore[attr-defined]
    ...
    async def _validate_message_media(self, value: Callable[[str], Any]) -> None:
        ...
        if kind in {"image", "audio"}:
            if media_id is None:
                raise ValidationError("image and audio messages require finalized media")
            if recalled_at is not None:
        ...
        if media_id is not None:
            raise ValidationError("only image and audio messages may attach media")
