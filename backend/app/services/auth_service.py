...
class AuthService:
    ...
    async def login(self, member_key: str, password: str, installation_id: str, device_name: str | None = None) -> tuple[str, str, UUID, UUID]:
        ...
        access, refresh, device_id = await self.issue_session(
            member, installation_id, device_name=device_name,
            reinstate_revoked_device=True,
        )
        return access, refresh, device_id, member.id
    ...
    async def issue_session(
        ...
        issued_session = RefreshSession(member_id=member.id, device_id=device.id, token_hash=token_hash(refresh), expires_at=current_time + timedelta(seconds=self.settings.refresh_token_ttl_seconds))
        self.session.add(issued_session)
        await self.session.flush()
        access = encode_access_token(member.id, device.id, issued_session.id, self.settings.access_token_secret, self.settings.access_token_ttl_seconds)
    ...
    async def rotate(self, presented_token: str) -> tuple[str, str, UUID, UUID]:
        ...
        access = encode_access_token(current.member_id, current.device_id, next_session.id, self.settings.access_token_secret, self.settings.access_token_ttl_seconds)
        return access, replacement, current.device_id, current.member_id

    async def revoke(self, presented_token: str) -> tuple[UUID, UUID] | None:
        ...
        if current is not None and current.revoked_at is None:
            ...
            return current.member_id, current.device_id
        return None
