...
class MemberRemovalService:
    ...
    async def request_removal(self, requester_id: UUID, target_member_id: UUID) -> MemberRemovalRequest:
        ...
        await self.repository.lock_members({requester_id, target_member_id})
        if await self.repository.active_member(requester_id) is None:
            raise ForbiddenError("requester is inactive")
        target = await self._removable_active_member(target_member_id, lock=False)
    ...
    async def decide(self, request_id: UUID, approver_id: UUID, decision: str) -> tuple[MemberRemovalRequest, int | None]:
        ...
        # Read immutable participant IDs without a row lock, then acquire every
        # Member lock in one order. Never hold a Request lock while waiting for
        # a Member: voluntary leave also closes pending requests.
        candidate = await self.repository.request(request_id)
        if candidate is None:
            raise NotFoundError("member removal request not found")
        await self.repository.lock_members({candidate.requester_id, candidate.target_member_id, approver_id})
        request = await self.repository.request(request_id, lock=True)
        if request is None or (request.requester_id, request.target_member_id) != (candidate.requester_id, candidate.target_member_id):
            raise ConflictError("member_removal_request_changed")
        ...
        if await self.repository.active_member(approver_id) is None:
            raise ForbiddenError("approver is inactive")
        if await self.repository.active_member(request.requester_id) is None:
            raise ConflictError("member_removal_requester_is_not_active")
        target = await self._removable_active_member(request.target_member_id, lock=False)
