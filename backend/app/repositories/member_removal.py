...
class MemberRemovalRepository:
    ...
    async def lock_members(self, member_ids: set[UUID]) -> None:
        """Acquire all participant rows in UUID order before any request row."""
        for member_id in sorted(member_ids):
            await self.session.scalar(select(Member).where(Member.id == member_id).with_for_update())
    ...
    async def request(self, request_id: UUID, *, lock: bool = False) -> MemberRemovalRequest | None:
        ...
        if lock:
            # The unlocked lookup may already be in this session's identity map.
            # Refresh its status after waiting for the row lock.
    ...
    async def cancel_pending_for_member(self, member_id: UUID, now: datetime) -> None:
        ...
        request_ids = (await self.session.scalars(
            select(MemberRemovalRequest.id)
            .where(
                MemberRemovalRequest.status == "pending",
                or_(
            ...
            .order_by(MemberRemovalRequest.id)
        )).all()
        for request_id in request_ids:
            request = await self.request(request_id, lock=True)
            if request is not None and request.status == "pending" and member_id in {request.target_member_id, request.requester_id}:
