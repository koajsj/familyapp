...
class MediaRepository:
    ...
    async def get_including_deleted(self, media_id: UUID) -> MediaAsset | None:
        return await self.session.get(MediaAsset, media_id)
    ...
    async def active_object_keys(self, retention_cutoff: datetime) -> set[str]:
        # Keep purged attachments through the offline-device tombstone window.
        statement = select(MediaAsset.object_key).where(
            (MediaAsset.deleted_at.is_(None)) | (MediaAsset.deleted_at > retention_cutoff)
        )
    ...
    async def has_active_family_chat_attachment(self, asset: MediaAsset) -> bool:
        ...
        statement = (
            ...
            .where(
                ...
                Message.kind.in_(("image", "audio", "file")),
