...
def encode_access_token(member_id: UUID, device_id: UUID, session_id: UUID, secret: str, ttl_seconds: int) -> str:
    now = datetime.now(UTC)
    header = {"alg": "HS256", "typ": "JWT"}
    payload = {"sub": str(member_id), "device": str(device_id), "session": str(session_id), "iat": int(now.timestamp()), "exp": int((now + timedelta(seconds=ttl_seconds)).timestamp()), "iss": "familyapp"}
...
def decode_access_token(value: str, secret: str) -> dict[str, Any] | None:
    try:
        ...
        if decoded.get("iss") != "familyapp" or int(decoded["exp"]) <= int(datetime.now(UTC).timestamp()):
            return None
        UUID(str(decoded["sub"])); UUID(str(decoded["device"])); UUID(str(decoded["session"]))
        return decoded
