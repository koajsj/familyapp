...
class Settings:
    ...
    def validate_production(self) -> None:
        if self.app_env.strip().lower() != "production":
            return
        secret = self.access_token_secret.strip()
        lowered = secret.lower()
        weak_markers = ("unconfigured", "replace", "example", "changeme", "change-me", "your-", "password", "qwer1234", "test", "placeholder")
        if len(secret.encode("utf-8")) < 32 or any(marker in lowered for marker in weak_markers) or len(set(secret)) < 8:
            raise ValueError("production JWT_SECRET must be a strong, non-placeholder secret")

    @classmethod
    def from_environment(cls) -> "Settings":
        if os.environ.get("APP_ENV", "development").strip().lower() == "production" and not os.environ.get("JWT_SECRET", "").strip():
        ...
        settings = cls(
            ...
            # Zero disables only pull-time acknowledged-prefix pruning. Daily
            # location privacy retention still expires changes after 30 days.
